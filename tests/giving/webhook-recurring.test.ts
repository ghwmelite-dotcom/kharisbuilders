...
import { createPendingSubscription, activateSubscription, getSubscriptionByCode } from '../../src/lib/db/subscriptions';
...
describe('recurring webhooks', () => {
  ...
  it('attributes a cycle charge with NO plan_code to the giver\'s sole active subscription', async () => {
    // isolated giver with exactly one active subscription
    await createPendingSubscription(ctx.db, { local_ref: 'kb_sub_solo', customer_email: 'solo@x.com', plan_id: null, plan_code: 'PLN_SOLO', amount: 5000, interval: 'monthly', fund_id: fundId });
    await activateSubscription(ctx.db, { customerEmail: 'solo@x.com', planCode: 'PLN_SOLO', subscriptionCode: 'SUB_SOLO', emailToken: 't', customerCode: 'C', nextPaymentAt: null });
    await handlePaystackEvent(ctx.db, {
      event: 'charge.success',
      data: { reference: 'kb_solo_cycle', amount: 5000, currency: 'GHS', channel: 'card', status: 'success', customer: { email: 'solo@x.com' } /* no plan */ },
    });
    const d = await getDonationByReference(ctx.db, 'kb_solo_cycle');
    expect(d?.fund_id).toBe(fundId);
    expect(d?.type).toBe('recurring');
  });
